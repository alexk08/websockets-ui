...
import { handleAddShips, handleAddUserToRoom, handleAttack, handleCreateRoom, handleRegistration } from './handlers';
import { AddShipData, AttackInData, PlayerData, Room } from '../types';
...
export const handleWsConnection = (wsClient: WebSocket, req: IncomingMessage) => {
  ...
  wsClient.on('message', function message(rawData) {
    ...
    if (type === 'attack') handleAttack({ attackData: data as AttackInData, clientId });
  });
